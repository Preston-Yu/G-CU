...
Send a single JSON line (terminated by CRLF) to `tcp://<board_ip>:22345`. Example for hardware model `v2.1`:
...
1. `model` must match the firmware `kExpectedModel` (currently `v2.1`).
2. `analog` and `select` arrays may only contain pins from that model's whitelist (max 11 analog pins, 13 select pins).
