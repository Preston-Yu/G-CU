...
const char* kManifestUrl = "https://isensing-s1.u-aizu.ac.jp/OTA/manifest.json";
...
bool fetchManifestPayload(String& payload) {
  for (uint8_t attempt = 1; attempt <= kManifestMaxAttempts; ++attempt) {
    ...
    LOGI("OTA", "manifest_fetch attempt=%u/%u url=%s",
         attempt, kManifestMaxAttempts, kManifestUrl);
    if (!http.begin(client, kManifestUrl)) {
      LOGE("OTA", "manifest_http_init status=fail");
    } else {
      ...
      LOGE("OTA", "manifest_http_error code=%d", httpCode);
      http.end();
    }
    if (attempt < kManifestMaxAttempts) {
      uint32_t backoff = kManifestRetryDelayMs * attempt * attempt;
      LOGW("OTA", "manifest_retry delay_ms=%u", backoff);
      delay(backoff);
      WiFi.reconnect();
    }
  }
...
bool findManifestForModel(const String& json, const char* targetModel, ManifestInfo& manifest) {
  ...
  if (devicesIdx < 0) {
    LOGE("OTA", "manifest_missing_devices_array");
    return false;
  }
  int arrayStart = json.indexOf('[', devicesIdx);
  int arrayEnd = json.indexOf(']', arrayStart);
  if (arrayStart < 0 || arrayEnd < 0) {
    LOGE("OTA", "manifest_devices_malformed");
    return false;
  }
  ...
  while (cursor < arrayEnd) {
    ...
    if (!extractField(json, objStart, "latest", manifest.latest)) {
      LOGE("OTA", "manifest_missing_field field=latest");
      return false;
    }
    if (!extractField(json, objStart, "url", manifest.url)) {
      LOGE("OTA", "manifest_missing_field field=url");
      return false;
    }
    if (!extractField(json, objStart, "sha256", manifest.sha256)) {
      LOGE("OTA", "manifest_missing_field field=sha256");
      return false;
    }
    return true;
  }
  LOGW("OTA", "manifest_model_not_found model=%s", targetModel ? targetModel : "null");
  return false;
}
...
bool downloadAndFlash(const ManifestInfo& manifest) {
  ...
  LOGI("OTA", "download start url=%s", manifest.url.c_str());
  if (!http.begin(client, manifest.url)) {
    LOGE("OTA", "firmware_http_init status=fail");
    otaLedError();
    return false;
  }
  int httpCode = http.GET();
  if (httpCode != HTTP_CODE_OK) {
    LOGE("OTA", "firmware_http_error code=%d", httpCode);
    http.end();
    otaLedError();
    return false;
  }
  ...
  if (totalLength > 0) {
    LOGI("OTA", "firmware_size bytes=%d", totalLength);
  } else {
    LOGW("OTA", "firmware_size unknown");
  }
  if (!Update.begin(totalLength > 0 ? totalLength : UPDATE_SIZE_UNKNOWN)) {
    LOGE("OTA", "update_begin_failed");
    Update.printError(logErrorPrint());
    http.end();
    otaLedError();
    return false;
  }
  ...
  while (http.connected()) {
    ...
    if (Update.write(buffer, readBytes) != readBytes) {
      LOGE("OTA", "update_write_failed bytes=%d", readBytes);
      Update.printError(logErrorPrint());
    ...
    if (millis() - lastLog > 1000) {
      LOGI("OTA", "progress bytes=%u", static_cast<unsigned int>(written));
      lastLog = millis();
    }
  }

  if (totalLength > 0 && written != static_cast<size_t>(totalLength)) {
    LOGE("OTA", "length_mismatch expected=%d got=%u", totalLength, static_cast<unsigned int>(written));
    mbedtls_sha256_free(&ctx);
    http.end();
    otaLedError();
    return false;
  ...
  if (!shaMatches(manifest.sha256, actualSha)) {
    LOGE("OTA", "sha_mismatch expected=%s actual=%s", manifest.sha256.c_str(), actualSha.c_str());
    otaLedError();
    return false;
  }

  if (!Update.end(true)) {
    LOGE("OTA", "update_end_failed");
    Update.printError(logErrorPrint());
    otaLedError();
    return false;
  }
  if (!Update.isFinished()) {
    LOGE("OTA", "update_not_finished");
    otaLedError();
    return false;
  }

  LOGI("OTA", "update_success rebooting=1");
  otaLedSuccess();
  delay(100);
  ESP.restart();
  return true;
...
bool runOnlineOtaUpdateIfAvailable(const char* currentVersion) {
  if (!g_wifiGotIP) {
    LOGW("OTA", "skip reason=wifi_not_ready");
    otaLedIdle();
    return false;
  }
  if (!currentVersion) {
  ...
  if (cmp <= 0) {
    LOGI("OTA", "no_update current=%s remote=%s", currentVersion, manifest.latest.c_str());
    otaLedIdle();
    return false;
  }

  LOGI("OTA", "update_available from=%s to=%s", currentVersion, manifest.latest.c_str());
